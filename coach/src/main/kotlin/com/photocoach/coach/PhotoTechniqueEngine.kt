...
object PhotoTechniqueEngine {
    fun suggest(signals: Signals, capabilities: TechniqueCapabilities): TechniqueSuggestion? {
        ...
        return when {
            signals.subjectCutOff -> suggestion(CueId.P1_FRAME, TechniqueCategory.COMPOSITION, "人物往画面里站一点", "人物贴近画面边缘")
            ...
            signals.meanLuma?.let { it < 55f } == true && !signals.handheldStable ->
                suggestion(CueId.P1_NIGHT, TechniqueCategory.NIGHT, "扶稳手机，再拍一张", "暗光下手持不稳定")
            signals.subjectMotionHigh -> suggestion(if (capabilities.burstEnabled) CueId.P1_MOTION_BURST else CueId.P1_MOTION, TechniqueCategory.MOTION,
    ...
    private fun multiPersonSuggestion(signals: Signals): TechniqueSuggestion? = when {
        ...
        kotlin.math.abs(signals.tiltDegrees) > 3f -> suggestion(CueId.P1_MULTI_LEVEL, TechniqueCategory.MULTI_PERSON_SAFE, "沿网格放平手机", "多人画面倾斜")
        signals.skyOverexposed && signals.exposureReductionAllowed && !signals.faceDarkerThanScene -> suggestion(CueId.P1_MULTI_HIGHLIGHTS, TechniqueCategory.MULTI_PERSON_SAFE, "曝光降一点", "多人画面高光溢出")
        else -> null
    }

    private fun suggestion(cueId: CueId, category: TechniqueCategory, text: String, reason: String) =
        TechniqueSuggestion(cueId, category, text, reason, Audience.SHOOTER)
}
